...
def compute_turnover_stats(df: pd.DataFrame) -> pd.DataFrame:
    """Per brand+model compute avg_days_to_sell and weekly_turnover.
    ...
    if df.empty or "first_seen_at" not in df.columns:
        return pd.DataFrame(columns=["brand", "model", "avg_days_to_sell", "weekly_turnover"])

    inactive = df[df["is_active"] == False].copy()

    if inactive.empty:
        total = df.groupby(["brand", "model"]).size().reset_index(name="total_listings")
        total["avg_days_to_sell"] = pd.NA
        total["weekly_turnover"] = 0.0
        return total[["brand", "model", "avg_days_to_sell", "weekly_turnover"]]
    ...
    avg_sell = (
        inactive.groupby(["brand", "model"])["duration_days"]
        .mean()
        .round(1)
    ...
    weekly = recent.groupby(["brand", "model"]).size().reset_index(name="sold_last_week")

    total = df.groupby(["brand", "model"]).size().reset_index(name="total_listings")

    result = avg_sell.merge(weekly, on=["brand", "model"], how="left")
    result = result.merge(total, on=["brand", "model"], how="left")
    ...
    return result[["brand", "model", "avg_days_to_sell", "weekly_turnover"]]
