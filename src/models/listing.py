...
class Listing(Base):
    ...
    fuel_type = Column(String)             # Combustivel: Diesel, Gasolina, Hibrido, Electrico, GPL
    horsepower = Column(Integer)           # Potencia (cv)
    transmission = Column(String)          # Tipo de Caixa: Automatica, Manual
    segment = Column(String)               # Segmento: SUV/TT, Berlina, Coupe, Monovolume...
    doors = Column(String)                 # Portas: 1-3, 4-5
    seats = Column(Integer)               # Lugares
    color = Column(String)                 # Cor
    condition = Column(String)             # Condicao: Usado, Novo, Para pecas
