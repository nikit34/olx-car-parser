...
model whose name AutoScout24 does not know — the Portuguese vocabulary has
"Peugeot 308 SW" and "Mini 3 Portas", which are body variants there rather than
models — comes back empty once and is then dropped for the rest of the run
instead of spending a request on every one of its years.
    ...
    if args.dry_run:
        for brand, model, year, n in pending[:40]:
            print(f"       {brand} {model} {year}  (PT n={n})  "
                  f"→ /lst/{slugify(brand)}/{slugify(model)} re_{year}")
        return 0
    ...
    with AutoScoutClient(config=config) as client:
        for brand, model, year, _n in pending:
            ...
            if (brand, model) in unmapped:
                continue
            try:
                listings = client.model_year(slugify(brand), slugify(model), year,
