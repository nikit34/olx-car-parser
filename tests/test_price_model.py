...
def test_train_returns_model_and_metrics():
    ...
    models, cat_maps, metrics, oof_preds, calibrator, _unc, *_imp = result
...
def test_predict_prices_uses_uncertainty_when_present(monkeypatch):
    ...
    result = train_price_model(df)
    assert result is not None
    models, cat_maps, metrics, oof_preds, calibrator, uncertainty, *_imp = result
    assert uncertainty is not None
...
def test_predict_prices_falls_back_to_per_bucket_q_when_uncertainty_none():
    ...
    result = train_price_model(df)
    assert result is not None
    models, cat_maps, metrics, _oof, calibrator, _unc, *_imp = result
    _conf_q = metrics.get("conformal_q", 0.0)
    _bucket_q = metrics.get("conformal_q_per_bucket", {})
    _edges = [tuple(e) for e in metrics.get("conformal_q_bucket_edges") or []]
...
def test_train_falls_back_to_random_cqr_without_first_seen_at():
    ...
    _models, _maps, metrics, _oof, _calib, _unc, *_imp = train_price_model(df)
...
def test_predictions_are_positive():
    df = _sample_listings()
    models, cat_maps, _metrics, _oof, _calib, _unc, *_imp = train_price_model(df)
...
def test_oof_preds_used_for_known_olx_ids():
    ...
    models, cat_maps, _metrics, oof_preds, calibrator, _unc, *_imp = train_price_model(df)
...
def test_calibrator_changes_predictions_for_new_rows():
    ...
    models, cat_maps, _metrics, _oof, calibrator, _unc, *_imp = train_price_model(df)
...
def test_handles_missing_features():
    ...
    result = train_price_model(df)
    assert result is not None
    models, cat_maps, _metrics, _oof, _calib, _unc, *_imp = result
...
class TestFlipperPrimitivesInFeatureSet:
    ...
    def test_train_succeeds_on_data_missing_new_columns(self):
        ...
        result = train_price_model(df)
        assert result is not None
        _models, _cat_maps, metrics, _oof, _calib, _unc, *_imp = result
