...
def _sample_listings(n: int = 200, seed: int = 42) -> pd.DataFrame:
    ...
    _CONDITIONS = ["bom estado", "excelente", "muito bom", "primeiro dono",
                   "pouco usado", "diesel económico"]
    titles = [f"{b} {m} {y}" for b, m, y in zip(brands, models, years)]
    descriptions = [
        f"Vendo {b} {m} de {y}, {rng.choice(_CONDITIONS)}, ITV em dia."
...
def trained():
    ...
    return df, models, cat_maps, metrics, oof_preds, calibrator, text_pipeline


def test_evaluate_oof_global_metrics(trained):
    df, _models, _maps, _metrics, oof, _calib, _text = trained
...
def test_time_backtest_returns_one_row_per_fold(trained):
    df, _models, _maps, metrics, _oof, _calib, _text = trained
